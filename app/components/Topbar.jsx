...
const [token, setToken] = useState(Cookies.get("user"))
  ...
  return (
    <div className="py-4 flex justify-end gap-5 items-center">
      {token ? (
        <>
          <button className="text-center bg-[#3A3A3A] p-4 font-medium text-base h-min">
            Jobs
          </button>
          <button className="text-center bg-[#144D00] p-4 font-medium text-base h-min">
            Upload new CVs <br /> <span className="font-normal">10 tokens</span>
          </button>
          <button className="bg-white text-base rounded-full px-3 text-black py-3 h-min">
            Account icon
          </button>
        </>
      ) : (
